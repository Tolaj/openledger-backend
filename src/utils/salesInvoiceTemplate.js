...
export const renderSalesInvoiceHtml = (inv, group) => {
    ...
    if (tmpl === "receipt") return buildReceiptHtml({
        docType: "Sales Invoice", docNumber: inv.invoiceNumber, docDate: invoiceDate, status: inv.status,
        ...
        partyGstin: inv.customer?.gstin, partyEmail: inv.customer?.email, partyAddress: inv.customer?.address,
...
<div class="page">
  <div class="doc-header">
    <div class="brand" style="display:flex;align-items:center;gap:16px;">
      ...
      <div>${group?.businessDetails?.legalName || group?.name || "OpenLedger"}<span>Sales Invoice</span></div>
    </div>
    <div class="doc-meta">
  ...
  <div class="parties">
    ...
    <div class="party-card">
      ...
      ${inv.customer?.gstin ? `<div class="party-detail">GSTIN: ${inv.customer.gstin}</div>` : ""}
